    ...
    if (socketConnection) {
      ...
      socketConnection.on("conversation", (data) => {
        ...
        const conversationUserData = data.map((conversationUser, index) => {
          ...
          } else if (conversationUser.sender?._id === user?._id) {
            return {
              ...conversationUser,
          ...
          {allUser.map((conv, index) => {
            return (
              <NavLink
                ...
                key={conv._id}
                className="flex items-center gap-2 py-3 px-2 border border-transparent hover:border-primary rounded hover:bg-slate-200 cursor-pointer"
              >
                <div>
                  <Avatar
